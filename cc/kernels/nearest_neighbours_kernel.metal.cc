...
template <> const Eigen::MetalDevice &OpKernelContext::eigen_device() const {
...
template <typename T> struct NearestNeighboursFunctor<MetalDevice, T> {
  ...
  void operator()(const MetalDevice &device, const Tensor *token_embeddings,
    ...
    const auto function_name =
        NS::String::string("nearest_neighbours", NS::ASCIIStringEncoding);
    ...
    compute_encoder->setBytes(&num_tokens, sizeof(int32_t), 3);
    compute_encoder->setBytes(&vocab_size, sizeof(int32_t), 4);
    compute_encoder->setBytes(&embedding_dim, sizeof(int32_t), 5);
...
template struct NearestNeighboursFunctor<MetalDevice, float>;
} // namespace functor
} // namespace tensorflow
