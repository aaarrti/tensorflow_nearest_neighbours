...
std::vector<int32_t> findIndexOfNearestNeighbour(
    ...
    >>(
            token_embeddings->SubSlice(batch_index).flat<float>().data(),   /* ptr to data */
            vocab_size,                                                     /* num_rows */
    ...
    for (auto token_index = 0; token_index != sequence_length; token_index++) {
        ...
        for (auto matrix_row_index = 0; matrix_row_index != vocab_size; matrix_row_index++) {
            ...
            const auto dist = static_cast<float>((row - embedding).squaredNorm());
        ...
        auto argmin = static_cast<int32_t>(std::distance(std::begin(distances), it));
        argmin_vector[token_index] = argmin;
    }
    return argmin_vector;
}
...
public:
    ...
    void Compute(tensorflow::OpKernelContext *context) override {
        ...
        const auto output_shaped = output_tensor->shaped<float, 3>({batch_size, sequence_length, embedding_dim});
        ...
        auto compute_shard = [
                ...
                &embedding_dim,
                &eigen_embedding_matrix,
                &embedding_matrix_shaped
        ](int32_t start, int32_t stop) {

            for (auto batch_index = start; batch_index != stop; batch_index++) {
                auto argmin_vector = findIndexOfNearestNeighbour(token_embeddings, eigen_embedding_matrix,
                                                                 batch_index, vocab_size, embedding_dim,
                                                                 sequence_length);

                for (auto token_index = 0; token_index != argmin_vector.size(); token_index++) {
                    // Fill the output
                    for (auto i = 0; i != embedding_dim; i++) {
                        output_shaped({batch_index, token_index, i}) = embedding_matrix_shaped(
                                {argmin_vector[token_index], i});
                    }
                }
            }
        };
...
public:
    ...
    explicit NearestNeighboursIndexesOp(tensorflow::OpKernelConstruction *context) : tensorflow::OpKernel(
            context) {}
    ...
    void Compute(tensorflow::OpKernelContext *context) override {
        ...
        // Create output
        tensorflow::Tensor *output_tensor = nullptr;
        OP_REQUIRES_OK(context, context->allocate_output(0, tensorflow::TensorShape({batch_size, sequence_length}),
                                                         &output_tensor));
        // Reshape for indexing
        const auto output_shaped = output_tensor->shaped<int32_t, 2>({batch_size, sequence_length});
        ...
        auto compute_shard = [
            ...
            for (auto batch_index = start; batch_index != stop; batch_index++) {
                ...
                for (auto token_index = 0; token_index != argmin_vector.size(); token_index++) {
                    output_shaped({batch_index, token_index}) = argmin_vector[token_index];
                }
            }
        };
...
REGISTER_KERNEL_BUILDER(Name("NearestNeighboursIndexes").Device(tensorflow::DEVICE_CPU), NearestNeighboursIndexesOp)
