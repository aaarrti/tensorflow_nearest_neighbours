...
int nearest_neighbour_index(
    int vocab_size, const Eigen::Vector<T, Eigen::Dynamic> embedding,
  ...
  const auto argmin =
      static_cast<int>(std::distance(std::begin(distances), it));
  return argmin;
}
...
int index_3d_flat(int index_0, int index_1, int index_2, int shape_1,
                  int shape_2) {
...
struct NearestNeighboursFunctor<CPUDevice, T> {
  void operator()(const CPUDevice &device, int batch_size, int num_tokens,
