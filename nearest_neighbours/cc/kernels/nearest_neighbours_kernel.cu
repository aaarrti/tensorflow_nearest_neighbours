...
__device__ int index_3d_flat_cuda(int index_0, int index_1, int index_2,
                                  int shape_1, int shape_2) {
  return index_2 + index_1 * shape_2 + index_0 * shape_2 * shape_1;
}

// Define the CUDA kernel.
template <typename T>
__global__ void NearestNeighboursCudaKernel(const T *token_embeddings,
                                            const T *embedding_matrix,
                                            T *output, int num_tokens,
  ...
  for (int word_index = 0; word_index != vocab_size; word_index++) {
    ...
    for (int i = 0; i != embedding_dim; i++) {
      const int index_in_embedding_matrix =
          index_2d_flat_cuda(word_index, i, embedding_dim);
      const int index_in_token_embeddings = index_3d_flat_cuda(
          index_in_batch, index_in_sequence, i, num_tokens, embedding_dim);
  ...
  for (int i = 0; i != embedding_dim; i++) {
    const int index_in_output = index_3d_flat_cuda(
        index_in_batch, index_in_sequence, i, num_tokens, embedding_dim);
    const int index_in_embedding_matrix =
        index_2d_flat_cuda(argmin, i, embedding_dim);
...
struct NearestNeighboursFunctor<GPUDevice, T> {
  void operator()(const GPUDevice &device, int batch_size, int num_tokens,
                  int vocab_size, int embedding_dim, const T *token_embeddings,
                  const T *embedding_matrix, T *output) {
    NearestNeighboursCudaKernel<T>
        <<<batch_size, num_tokens>>>(token_embeddings, embedding_matrix, output,
