...
thread int index_2d_flat(const thread int &index_0, const thread int &index_1,
                         constant int &shape_1) {
  return index_1 + index_0 * shape_1;
}

thread int index_3d_flat(const thread int &index_0, const thread int &index_1,
                         thread int &index_2, constant int &shape_1,
                         constant int &shape_2) {
  return index_2 + index_1 * shape_2 + index_0 * shape_2 * shape_1;
}

kernel void nearest_neighbours(device float *token_embeddings [[buffer(0)]],
  ...
  for (int word_index = 0; word_index != vocab_size; word_index++) {
    ...
    for (int i = 0; i != embed_dim; i++) {
      const int index_in_embedding_matrix =
          index_2d_flat(word_index, i, embed_dim);
      const int index_in_token_embeddings = index_3d_flat(
          index_in_batch, index_in_sequence, i, num_tokens, embed_dim);
  ...
  for (int i = 0; i != embed_dim; i++) {
    const int index_in_output = index_3d_flat(index_in_batch, index_in_sequence,
                                              i, num_tokens, embed_dim);
