...
namespace tensorflow {
  namespace functor {
    ...
    namespace {
      __device__ __inline__ int index_2d_flat(int index_0, int index_1, int shape_1) {
        return index_1 + index_0 * shape_1;
      }

      __device__ __inline__ int index_3d_flat(int index_0, int index_1, int index_2, int shape_1, int shape_2) {
    ...
    struct NearestNeighboursFunctor<GPUDevice, T> {
      ...
      void operator()(const GPUDevice &device,
                      ParamsType params,
                      const T *token_embeddings,
                      const T *embedding_matrix, T
                      *output) {
        NearestNeighboursCudaKernel<T><<<params.batch_size, params.num_tokens>>>(token_embeddings,
                                                                                 embedding_matrix,
                                                                                 output,
                                                                                 params.num_tokens,
                                                                                 params.vocab_size,
    ...
    template
    struct NearestNeighboursFunctor<GPUDevice, double>;
  }
}