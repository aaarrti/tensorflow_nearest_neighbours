...
namespace tensorflow::functor {
  ...
  public:
    ...
    void Compute(tensorflow::OpKernelContext *context) override {
      ...
      const auto ndim = token_embeddings->dims();
      const auto vocab_size = static_cast<int>(embedding_matrix->dim_size(0));
      const auto embedding_dim = static_cast<int>(embedding_matrix->dim_size(1));
      ...
      switch (ndim) {
        ...
        default: {
          context->CtxFailure(errors::InvalidArgument("ndim = " + std::to_string(ndim)));
          break;
        }
      }
    }
  ...
  REGISTER_KERNEL_BUILDER(Name("NearestNeighboursIndexes")
                            .Device("CPU"), NearestNeighboursIndexesOp<CPUDevice, float>)
  REGISTER_KERNEL_BUILDER(Name("NearestNeighbours")
                            .Device("CPU"), NearestNeighboursOp<CPUDevice, float>)
